...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = transaction.outputSatoshis / 1e4;
  transformed.size = transaction.size; // can be virtual size (not equal to actual) if segwit
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / 1e4;
    transformed.fees = transaction.feeSatoshis / 1e4;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = input.satoshis / 1e4;
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    value: (output.satoshis / 1e4).toFixed(4),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
